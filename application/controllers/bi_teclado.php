	...
	{ 
		...
		if (ROL == SUPERROL) {
			...
			if ($this->form_validation->run() == FALSE ) {
					...
					$data['cargar_lista_teclados'] = $this->tbl_teclado_crud_model->buscar($teclado);
					#$data['cargar_lista_monitores'] = $this->tbl_teclado_crud_model->lista($id_teclado);					
					$data['cargar_empleados'] = $this->tbl_empleado_crud_model->cargar_empleados();					
					$this->load->view('header_view');
					$this->load->view('menu_view',$data);
					$this->load->view('teclado_view',$data);
			...
			if ($tiene_permiso == TRUE) {
				...
				$this->form_validation->set_rules('num_inventario', 'Número de Inventario', 'required|min_length[5]|max_length[10]|numeric');
					...
					if ($this->form_validation->run() == FALSE ) {
							...
							$data['cargar_lista_monitores'] = $this->tbl_teclado_crud_model->buscar($teclado);
							#$data['cargar_lista_monitores'] = $this->tbl_teclado_crud_model->lista($id_teclado);					
							$data['cargar_empleados'] = $this->tbl_empleado_crud_model->cargar_empleados();					
							$this->load->view('header_view');
							$this->load->view('menu_view',$data);
