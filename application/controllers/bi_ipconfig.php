...
class Bi_ipconfig extends CI_Controller {
	...
	public function index($id_ipconfig = false)
	{ 
		...
		if (ROL == SUPERROL) {
			...
			$data['ipconfigs'] = $this->db->list_fields('tbl_ipconfig');
			
			$data['cargar_ipconfigs'] = $this->tbl_ipconfig_crud_model->lista($id_ipconfig);  //aqui ejecuto el metodo 'cargar_users' de la clase ''tbla_user_crud_model
			$data['cargar_cpus_lista'] = $this->tbl_cpu_crud_model->get_cpu();
			
			$this->load->view('header_view');
			$this->load->view('menu_view',$data);
			$this->load->view('ipconfig_view',$data);
		...
		else
		{
			...
			if ($tiene_permiso == TRUE) {
				...
				$data['rol'] = ROL;
		 		...
		 		$data['cargar_ipconfigs'] = $this->tbl_ipconfig_crud_model->lista();  //aqui ejecuto el metodo 'cargar_users' de la clase ''tbla_user_crud_model
				$data['cargar_cpus_lista'] = $this->tbl_cpu_crud_model->get_cpu();
	...
	public function crear()
	{
		...
		if (ROL == SUPERROL) {
			...
			$nuevo = $this->tbl_ipconfig_crud_model->agrega($ip_config);

			if ( isset($nuevo) && is_int($nuevo) > 0 ) {
				
				redirect(base_url('ipconfigs'));
	...
	public function actualizar()
	{
		...
		if (ROL == SUPERROL) {
			...
			$ip_config = array(
				'id_ipconfig' => $this->input->post('id_ipconfig'),
			...
			$modificado = $this->tbl_ipconfig_crud_model->actualiza($ip_config);
	...
	public function eliminar($id_ipconfig = false)
	{
		// Si tienes Rol de SuperAdministrador entras sin permisos
		if (ROL == SUPERROL) {
			...
			$ip_config = array('id_ipconfig' => $id_ipconfig);

			$eliminado = $this->tbl_ipconfig_crud_model->elimina($ip_config);
