...
class Bi_ram extends CI_Controller {
	...
	public function index($id_ram = false)
	{ 
		...
		if (ROL == SUPERROL) {
			...
			$data['fields'] = $this->db->list_fields('tbl_ram');
			
			$data['cargar_lista'] = $this->tbl_ram_crud_model->lista($id_ram);  //aqui ejecuto el metodo 'cargar_users' de la clase ''tbla_user_crud_model
			$data['cargar_cpus_lista'] = $this->tbl_cpu_crud_model->get_cpu();
			
			$this->load->view('header_view');
			$this->load->view('menu_view',$data);
			$this->load->view('ram_view',$data);
		...
		else
		{
			...
			if ($tiene_permiso == TRUE) {
				...
				$data['rol'] = ROL;
		 		...
		 		$data['cargar_lista'] = $this->tbl_ram_crud_model->lista();  //aqui ejecuto el metodo 'cargar_users' de la clase ''tbla_user_crud_model
				$data['cargar_cpus_lista'] = $this->tbl_cpu_crud_model->get_cpu();
	...
	public function crear()
	{
		...
		if (ROL == SUPERROL) {
			...
			$memoria_ram = array(
				...
				'pines' => $this->input->post('pines'),
				'capacidad' => $this->input->post('capacidad'),
				'id_cpu' => $this->input->post('id_cpu'),
				'status' => $this->input->post('status')
			);			

			$nuevo = $this->tbl_ram_crud_model->agrega($memoria_ram);

			if ( isset($nuevo) && is_int($nuevo) > 0 ) {
				
				redirect(base_url('memorias_ram'));
	...
	public function actualizar()
	{
		...
		if (ROL == SUPERROL) {
			...
			$modificado = $this->tbl_ram_crud_model->actualiza($memoria_ram);
	...
	public function eliminar($id_ram = false)
	{
		// Si tienes Rol de SuperAdministrador entras sin permisos
		if (ROL == SUPERROL) {
			...
			$memoria_ram = array('id_ram' => $id_ram);

			$eliminado = $this->tbl_ram_crud_model->elimina($memoria_ram);
